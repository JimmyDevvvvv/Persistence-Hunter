...
function getTypeBadge(entryType, iocNotes) {
    // Try to detect IFEO or Run Key from ioc_notes / reg_path
...
// Build a compact chain arrow string: reg.exe → IFEO debugger key
...
function EntryListItem({ alert, active, onClick, index }) {
    ...
    const score = SEV_SCORE[alert.severity] ?? 0
    ...
    return (
        ...
        >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                ...
                <ScoreBox score={score} severity={alert.severity} />
...
function AlertDetail({ alert }) {
    ...
    const chainLine = chain.length > 0
    ...
    if (alert.ioc_notes?.includes('backdoor')) anomalies.push('suspicious_path')
    ...
    return (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', animation: 'fadeIn 0.15s ease-out' }}>
            ...
            <div style={{ padding: '14px 20px 12px', borderBottom: '1px solid var(--bg-border)', flexShrink: 0 }}>
                ...
                {/* Row 1: score + name + type + technique */}
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
                ...
                {tab === 'chain' && (
                    <ProcessTree chain={chain} loading={chainLoading} error={null} />
                )}
