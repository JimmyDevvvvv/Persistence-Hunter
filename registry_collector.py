    ...
    def _assess_severity(self, name: str, value: str) -> tuple[str, str]:
        ...
        for lol in LOLBINS:
            if lol in value_lower:
                # powershell and cmd are very common legit entries too
                if lol in ("powershell.exe", "cmd.exe"):
                    if any(x in value_lower for x in ["-enc", "-encodedcommand", "-nop", "-w hidden", "bypass"]):
                        notes.append(f"LOLBin with suspicious flags: {lol}")
                        return "critical", "; ".join(notes)
                    else:
                        notes.append(f"LOLBin present: {lol}")
                else:
                    notes.append(f"LOLBin: {lol}")
                    return "critical", "; ".join(notes)
        ...
        # Encoded content
        if "-enc" in value_lower or "-encodedcommand" in value_lower:
            notes.append("Base64-encoded command detected")
            return "critical", "; ".join(notes)

        # Check for known legit paths
